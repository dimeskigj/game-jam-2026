...
public partial class InventoryUI : CanvasLayer
{
	...
	public override void _Ready()
	{
		...
		var restartBtn = GetNodeOrNull<Button>("GameOverLabel/RestartButton");
		if (restartBtn != null)
		{
	...
	{
		// Find Player Controller to handle logic cleanly
		var window = GetTree().Root;
		for (int i = 0; i < window.GetChildCount(); i++)
		{
			var sceneRoot = window.GetChild(i);
			Node playerNode = sceneRoot.FindChild("Player", true, false);
